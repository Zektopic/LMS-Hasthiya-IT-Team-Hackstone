...
void main() {
  testWidgets('MyLearningView tabs interaction updates empty state', (
    WidgetTester tester,
  ) async {
    // Build our app and trigger a frame.
    await tester.pumpWidget(
      const MaterialApp(home: Scaffold(body: MyLearningView())),
    );

    // Verify initial state (In Progress tab)
    expect(find.text('Start Learning'), findsOneWidget);
    expect(
      find.text('Your enrolled courses and progress\nwill appear here.'),
      findsOneWidget,
    );
    ...
    expect(
      find.text('Keep learning! Your completed\ncourses will appear here.'),
      findsOneWidget,
    );
